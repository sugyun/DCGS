...
# GRASP method
#
def fvs_grasp(directed_graph, max_iter=2000, keep_self_loops=True):
...
def _restricted_candidate_list(directed_graph, alpha):
	...
	G = np.multiply([val for (node, val) in directed_graph.in_degree(nodelist)],
                  [val for (node, val) in directed_graph.out_degree(nodelist)])
	Gmin = min(G)
	Gmax = max(G)
